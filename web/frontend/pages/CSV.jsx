...
// ================================== FINAL 3 PAGE CSV FILE

import React, { useState, useEffect } from "react";
import "./OrdersCSV.css";
import * as XLSX from "xlsx";

// OrdersCSV_expanded.jsx
// Replacement for your OrdersCSV component that generates the detailed CSV
// (per-line-item) with all fields you asked for.
...
function buildRowsForOrder(order) {
  ...
  // If you have a vendor mapping in marketplace layer, insert here.
  // For now we'll read vendor info from each line_item.vendor (if present)
...
function OrdersCSV() {
  ...
  // Export CSV for a single order (all line items rows)
  // replace existing generateOrderCSV with this:
  const generateOrderCSV = (order) => {
...
//     rows.push([
//       order.shopify_order_id || order._id || "",
//       formatDateISO(orderDate),
//       formatDateISO(paymentDate),
...
//   // Export CSV for a single order (all line items rows)
//   const generateOrderCSV = (order) => {
//     const rows = buildRowsForOrder(order);
//     downloadCSV(CSV_HEADERS, rows, `order-${order.order_number || order._id}.csv`);
//   };
